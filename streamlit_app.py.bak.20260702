...
def fetch_index_quotes():
    ...
    try:
        ...
        for d in diffs:
            code = d.get('f12', '')
            name = d.get('f14', '')
            price = (d.get('f2', 0) or 0) / 100
            chg = (d.get('f3', 0) or 0) / 100
...
def fetch_alpha_feed_quotes():
    ...
    try:
        ...
        resp = requests.get(url, headers={'X-API-Key': AF_API_KEY}, timeout=30)
    ...
    try:
        ...
        for d in diffs:
            results.append({
                'code': d.get('f12', ''),
                'name': d.get('f14', ''),
                'current': (d.get('f2', 0) or 0) / 100,
                'changePercent': (d.get('f3', 0) or 0) / 100,
                'change': (d.get('f4', 0) or 0) / 100,
                'volume': d.get('f5', 0) or 0,
                'amount': d.get('f6', 0) or 0,
                'amplitude': (d.get('f7', 0) or 0) / 100,
                'turnover': (d.get('f8', 0) or 0) / 100,
                'vol_ratio': (d.get('f10', 0) or 0) / 100,
                'open': (d.get('f17', 0) or 0) / 100,
                'prevClose': (d.get('f18', 0) or 0) / 100,
                'high': (d.get('f15', 0) or 0) / 100,
                'low': (d.get('f16', 0) or 0) / 100,
                'pe': (d.get('f9', 0) or 0) / 100,
...
def run_scan(board: str, min_chg: float, max_chg: float, min_vr: float,
    ...
    # Step 1: 获取全市场行情
    progress.progress(10, "获取AlphaFeed全市场行情...")
    all_stocks = fetch_alpha_feed_quotes()

    if not all_stocks:
        # 回退到东方财富
        progress.progress(10, "AlphaFeed不可用，回退到东方财富...")
        all_stocks = []
        for page in range(1, 6):
            ...
            progress.progress(10 + page * 5, f"获取第{page}页数据...")
...
def render_scan_page():
    ...
    if results:
        ...
        for i, r in enumerate(results):
            ...
            signal_tags = ''.join([
                f'<span class="tag-{s["type"]}">{s["label"]}</span> '
                for s in signals[:3]
            ])
            chg = r.get('changePercent', 0) or 0
            chg_class = 'cell-red' if chg >= 0 else 'cell-green'
            flow = r.get('sector_flow')
            flow_str = f"{flow/1e4:.2f}亿" if flow else '--'
            ...
            df_data.append({
                ...
                '名称': r['name'],
                '现价': r.get('current', '--'),
                '涨幅': f'<span class="{chg_class}">{chg:+.2f}%</span>',
                '量比': f"{r.get('vol_ratio', '--'):.2f}" if r.get('vol_ratio') else '--',
                '换手率': f"{r.get('turnover', '--'):.1f}%" if r.get('turnover') else '--',
                '成交额': f"{r.get('amount', 0)/1e8:.2f}亿" if r.get('amount') else '--',
                '主力净流入': flow_str,
                '策略信号': signal_tags,
                '评分': r.get('score', 0),
            })
